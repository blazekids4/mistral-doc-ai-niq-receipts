...
def get_blob_service_client(account_name):
    """Get a blob service client using DefaultAzureCredential"""
...
def get_all_blobs_with_prefix():
    """Get all receipt blobs from the single-capture folder and return their names"""
    storage_account_name, container_name, configured_prefix = load_environment_variables()

    print(f"Storage account: {storage_account_name}")
    print(f"Container name: {container_name}")

    # Use configured prefix if present; otherwise default to receipts/single-capture/
    default_prefix = "receipts/single-capture/"
    receipts_prefix = configured_prefix or default_prefix
    ...
    try:
        ...
        receipt_blobs = [blob.name for blob in container_client.list_blobs(name_starts_with=receipts_prefix)]

        if not receipt_blobs:
            ...
            if "receipts/" in unique_prefixes:
                print(" - Try using just 'receipts/' as the prefix")
            if any("receipt" in p.lower() for p in unique_prefixes):
                alternatives = [p for p in unique_prefixes if "receipt" in p.lower()]
                for alt in alternatives:
                    print(f" - Try using '{alt}' as the prefix")

            # If no receipt-related prefixes, suggest checking top-level blobs
            if not any("receipt" in p.lower() for p in unique_prefixes):
                print(" - Try looking at top-level blobs (empty prefix)")

            # List all blobs for better debugging
            list_all_blobs_in_container()

        return receipt_blobs
    except Exception as e:
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("1. Looking for receipts using configured prefix (or default)...")
        blob_names = get_all_blobs_with_prefix()
        print(f"\nFound {len(blob_names)} receipt files using configured/default prefix")
        ...
        if blob_names:
            ...
            print(f"Successfully encoded receipt {first_blob} to base64")
            print(f"Base64 string length: {len(base64_data)}")
        else:
            print("\n2. No receipts found with default prefix. Listing all blobs...")
            list_all_blobs_in_container()
